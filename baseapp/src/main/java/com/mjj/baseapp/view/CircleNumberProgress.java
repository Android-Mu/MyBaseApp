...
import com.mjj.baseapp.utils.BaseUtil;

/**
 * 文件描述：自定义带有百分比的环形进度条
 * <p>
 * Created by Mjj on 2016/8/31.
 */
public class CircleNumberProgress extends View {
     ...
     * 文字百分比的字体大小（sp）
     */
    private int paintTextSize = 17;

    /**
     * 未完成进度条的颜色
     */
    private int paintUndoneColor = 0xffecedf1;

    /**
     * 已完成进度条的颜色
     */
    private int paintDoneColor = 0xfff95353;

    /**
     * 百分比文字的颜色
     */
    private int paintTextColor = 0xfff95353;
    ...
    private void initData() {
        // 设置进度条画笔的宽度
        paintProgressWidthPx = BaseUtil.dip2px(context, paintProgressWidth);

        // 设置文字画笔的尺寸(px)
        paintTextSizePx = BaseUtil.sp2px(context, paintTextSize);
...
//        canvas.drawTextOnPath(text, path, 0, 0, paintText);
        ...
        // 绘制文字在圆环的中心
        canvas.drawText(text, getMeasuredHeight() / 2 - textWidth / 2, getMeasuredWidth() / 2 + textHeight / 2 + 5, paintText);
